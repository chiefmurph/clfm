...
windows(width = 12, height = 6) #, canvas = "lightgray")
...
plot(alpha, LRfunction(x, y, alpha), type = "l", main = "12-24", col = "red")#, ylim = c(1.55, 1.75))
abline(h=tro$dpo["12", "ata"], lty = "dotted", col = "red")
abline(v=oa["12"], lty = "dotted", col = "red")
imin <- which.min(x)
imax <- which.max(x)
abline(h = ata(tro$tri)[names(x)[imin], 1], lty = "dashed", col = "lightblue")
abline(h = ata(tro$tri)[names(x)[imax], 1], lty = "dashed", col = "lightgreen")
a <- round(oa["12"], 2)
b <- round(tro$dpo["12", "ata"], 3)
text(a+5, b+.01, labels = paste("(", a, ", ", b, ")", sep = ""), adj = c(0,0))
#text(a+5, b+.01, labels = "Selected", adj = c(0,0), col = "red")

i <- i - 1
x <- tro$tri[as.character(i:(i+8)),"24"]
y <- tro$tri[as.character(i:(i+8)),"36"]
alpha <- seq(-100,100,by=.1)
plot(alpha, LRfunction(x, y, alpha), type = "l", main = "24-36", col = "red", ylim = c(1.5, 1.75))
abline(h=tro$dpo["24", "ata"], lty = "dotted", col = "red")
abline(v=oa["24"], lty = "dotted", col = "red")
imin <- which.min(x)
imax <- which.max(x)
abline(h = ata(tro$tri)[names(x)[imin], 2], lty = "dashed", col = "lightblue"); print(ata(tro$tri)[names(x)[imin], 2])
abline(h = ata(tro$tri)[names(x)[imax], 2], lty = "dashed", col = "lightgreen"); print(ata(tro$tri)[names(x)[imax], 2])
a <- round(oa["24"], 2)
...
alpha <- seq(-100,100,by=.1)
plot(alpha, LRfunction(x, y, alpha), type = "l", main = "36-48", col = "red")#, ylim = c(1.55, 1.75))
abline(h=tro$dpo["36", "ata"], lty = "dotted", col = "red")
abline(v=oa["36"], lty = "dotted", col = "red")
imin <- which.min(x)
imax <- which.max(x)
abline(h = ata(tro$tri)[names(x)[imin], 3], lty = "dashed", col = "lightblue")
abline(h = ata(tro$tri)[names(x)[imax], 3], lty = "dashed", col = "lightgreen")
a <- round(oa["36"], 2)
b <- round(tro$dpo["36", "ata"], 3)
text(a+5, b+.001, labels = paste("(", a, ", ", b, ")", sep = ""), adj = c(0,0))
#text(a+5, b+.01, labels = "Selected", adj = c(0,0), col = "red")
...
alpha <- seq(-100,100,by=.1)
plot(alpha, LRfunction(x, y, alpha), type = "l", main = "48-60", ylim = c(1.12, 1.16), col = "red")
abline(h=tro$dpo["48", "ata"], lty = "dotted", col = "red")
imin <- which.min(x)
imax <- which.max(x)
abline(h = ata(tro$tri)[names(x)[imin], 4], lty = "dashed", col = "lightblue")
abline(h = ata(tro$tri)[names(x)[imax], 4], lty = "dashed", col = "lightgreen")
a <- 0
...
triangle <- tro$tri
triangle <- triangle[as.character(2001:2012), 1:4]
triangle
library(ChainLadder)
delta <- CLFMdelta(Triangle = triangle, 
   selected = c(3.202, 1.628, 1.28))
delta[1] <- -8
MackChainLadder(triangle, alpha = 2 - delta, 
   est.sigma="Mack", mse.method = "Independence")
#(mcl <- MackChainLadder(triangle, alpha = 2 - oa, est.sigma="NA", mse.method = "Independence"))
